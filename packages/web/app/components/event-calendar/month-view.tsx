...
import { useEventVisibility } from './hooks/use-event-visibility.ts'
import { EventGap, EventHeight } from './constants.ts'
import {
	getAllEventsForDay,
	getEventsForDay,
...
import { DefaultStartHour } from '~/components/event-calendar/constants.ts'
		...
		>
			...
			<div className='grid flex-1 auto-rows-fr'>
				{weeks.map(week => {
					...
					const isFirstWeek = firstDay && days[0] && isSameDay(firstDay, days[0])
					return (
						<div
							key={firstDay ? firstDay.toISOString() : `week-${week.length}`}
						...
						>
							{week.map((day, dayIndex) => {
								...
								const isReferenceCell = isFirstWeek && dayIndex === 0
